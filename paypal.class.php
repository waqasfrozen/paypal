	...
	class MyPayPal {
		...
		function DoExpressCheckoutPayment(){
			...
			if(!empty(_SESSION('ppl_products'))&&!empty(_SESSION('ppl_charges'))){
				...
				$w_return['response'] = $httpParsedResponseAr ;

				if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])){
                    $w_return['ack'] = "success";
					if('Completed' == $httpParsedResponseAr["PAYMENTINFO_0_PAYMENTSTATUS"]){
                        $w_return['payment_status'] = "received";
					}
					elseif('Pending' == $httpParsedResponseAr["PAYMENTINFO_0_PAYMENTSTATUS"]){
                        $w_return['payment_status'] = "pending";
					}
                    $w_return['transaction_details'] = $this->GetTransactionDetails();
				}
				else{
                    $w_return['ack'] = "error";
				}
		...
		function GetTransactionDetails(){
            ...
            $w_return['checkout_details'] = $httpParsedResponseAr;
